...
export const History: FC = () => {
	...
	const s = pickSection(home, "history", locale as Locale);
	const str = (key: string, fallback: string) => (s?.[key] as string) || fallback;

	const milestones = useMemo(() => {
		const raw =
			(s?.history_items as
				| Record<string, { year?: string; title?: string; description?: string }>
				| undefined) ?? {};
		const api = Object.values(raw);

		if (!api.length) return content.milestones;
		return api.map((m) => ({
			year: m.year ?? "",
			title: { value: m.title ?? "" },
	...
	useGSAP(
		() => {
			...
			tl.to(q(".hs-sub"), { opacity: 1, letterSpacing: "0.25em", duration: 1 })
				...
				.to(
					q(".hs-stem"),
					{ scaleY: 1, duration: 0.7, stagger: 0.12, ease: "power2.out" },
					"<"
				)
